...
class TestZfsScaling(unittest2.TestCase):
        ...
        with patch.object(ExecuteNode, 'run', run_count) as p:
            with mocktime("20101112000000"):
                self.assertFalse(ZfsAutobackup(
                    "test test_target1 --no-progress --verbose --keep-source=10000 --keep-target=10000 --no-holds --allow-empty".split(
                        " ")).run())
    ...
    def test_manydatasets(self):
        ...
        with patch.object(ExecuteNode, 'run', run_count) as p:
            with mocktime("20101112000000"):
                self.assertFalse(ZfsAutobackup(
                    "test test_target1 --no-progress --verbose --no-holds --allow-empty".split(" ")).run())
