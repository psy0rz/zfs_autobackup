...
class TestZfsNode(unittest2.TestCase):
    ...
    def test_keepsource0target10queuedsend(self):
        ...
        # old behaviour, without bookmarks

        with mocktime("20101111000000"):
            self.assertFalse(ZfsAutobackup(
                "test test_target1 --no-progress --verbose --keep-source=0 --keep-target=10 --allow-empty --no-send --no-bookmarks".split(
                    " ")).run())
        ...
        with mocktime("20101111000002"):
            self.assertFalse(ZfsAutobackup(
                "test test_target1 --no-progress --verbose --keep-source=0 --keep-target=10 --allow-empty --no-bookmarks".split(
                    " ")).run())
